...
"  duncad-docs gl            OpenGL ES render layer + view modes\n"
...
"\n"
"AGENT:\n"
"  duncad-docs agent         How to use DunCAD as an AI agent\n"
"\n"
"PROJECT:\n"
...
"         Export > Export as STL  (win.export-stl)\n"
"         Quit\n"
"  Edit   Undo, Redo, Preferences\n"
"  View   Toggle Left Panel, Toggle Right Panel\n"
"  Insert (mutable — swaps between 3D shapes and EDA items per tab)\n"
"  Tools  OpenSCAD, KiCad\n"
"  Help   About, Documentation\n"
"\n"
"STL EXPORT (File > Export > Export as STL):\n"
"  Extracts isosurface from the current voxel grid via marching cubes\n"
"  and writes binary STL. Requires a rendered solid (use to_solid()).\n"
"  Opens a GTK4 file dialog for path selection.\n"
"  API: dc_scad_preview_export_stl(pv, path, &err_msg)\n"
"\n"
...
"\n"
"Over 100 commands organized by domain. Use subtopics for details:\n"
"\n"
"TOPICS:\n"
"  duncad-docs inspect commands core     2D editor, code editor, SCAD\n"
"  duncad-docs inspect commands gl       GL viewport, camera, render modes\n"
"  duncad-docs inspect commands voxel    Voxel/SDF, marching cubes, STL\n"
"  duncad-docs inspect commands bezier   Bezier mesh, 2D editor, primitives\n"
"  duncad-docs inspect commands eda      Schematic, PCB, EDA libraries\n"
"\n"
"RESPONSE FORMAT:\n"
...
static const char HELP_INSPECT_CMD_CORE[] =
"INSPECT: COMMANDS / CORE -- Editor, Code, Preview Commands\n"
"\n"
"READ COMMANDS:\n"
"  state                    Full state dump (JSON)\n"
"  render [path]            Render 2D canvas to PNG\n"
...
"  open_file <path>         Open a file in the code editor\n"
"  save_file [path]         Save (to path, or current file)\n"
"  select_lines <s> <e>    Select line range in editor\n"
"  insert_text <text>      Insert text at cursor\n"
"\n"
"SCAD/PREVIEW COMMANDS:\n"
"  render_scad <scad> [png]  Sync render SCAD file to PNG\n"
"  open_scad <path>          Launch OpenSCAD GUI\n"
"  preview_render            Trigger F5 render (Cubeiform -> viewport)\n"
"  render_status             Check if render is in progress\n"
"\n"
"CUBEIFORM COMMANDS:\n"
"  cubeiform_exec <src>      Execute Cubeiform source\n"
"  cubeiform_validate <src>  Parse without executing\n"
"\n"
"WINDOW/TAB COMMANDS:\n"
"  window_title <text>       Set window title\n"
"  window_status <text>      Set status bar text\n"
"  window_size               Get window dimensions\n"
"  tab <name>                Switch tab (3d_cad, eda)\n"
"  tab_state                 Get active tab\n"
"  shape_action <name>       Insert shape (cube, sphere, etc.)\n";

static const char HELP_INSPECT_CMD_GL[] =
"INSPECT: COMMANDS / GL -- GL Viewport Commands\n"
"\n"
"GL VIEWPORT COMMANDS:\n"
"  gl_state                  Viewport state JSON (camera, objects)\n"
"  gl_camera <dist> <yaw> <pitch>  Set camera position\n"
"  gl_reset                  Reset camera to default\n"
"  gl_ortho                  Toggle orthographic projection\n"
"  gl_grid                   Toggle grid\n"
"  gl_axes                   Toggle axes\n"
"  gl_select <index>         Select GL object by index\n"
"  gl_select_mode <mode>     Set selection mode\n"
"  gl_wireframe              Set wireframe render mode\n"
"  gl_blocky [0|1]           Set blocky (voxel) mode (toggle if no arg)\n"
"  gl_surface                Set smooth surface render mode\n"
"  gl_topo [scale]           Toggle topographic color mapping\n"
"  gl_load <stl_path>        Load STL into viewport\n"
"  gl_clear                  Clear all GL objects\n"
"  gl_capture [path]         Capture viewport to PNG\n"
"\n"
"TRANSFORM COMMANDS:\n"
"  transform_show <json>     Show transform panel with values\n"
"  transform_hide            Hide transform panel\n";

static const char HELP_INSPECT_CMD_VOXEL[] =
"INSPECT: COMMANDS / VOXEL -- Voxel, SDF, and Mesh Export\n"
"\n"
"VOXEL/SDF COMMANDS:\n"
"  voxel_sphere <args>       Create SDF sphere\n"
"  voxel_box <args>          Create SDF box\n"
"  voxel_csg <op>            CSG operation on voxel grids\n"
"  voxel_clear               Remove voxels from viewport\n"
"  voxel_state               Get voxel grid info (size, active count)\n"
"  voxel_resolution [n]      Get/set voxel resolution\n"
"\n"
"MESH EXPORT:\n"
"  marching_cubes [path]     Extract isosurface -> STL\n"
"                            Default: /tmp/duncad_mc.stl\n"
"\n"
"MESH DIAGNOSTIC (explicit-only, NEVER automatic):\n"
"  debug_render_mesh [path]  Run editor code through OpenSCAD\n"
"                            interpreter, write STL, load into\n"
"                            the MESH canvas with [DEBUG MESH]\n"
"                            banner. Sole sanctioned mesh\n"
"                            render path. Default path:\n"
"                            /tmp/duncad_debug_mesh.stl.\n"
"                            Banner clears on next preview_render.\n"
"\n"
"STL EXPORT PIPELINE:\n"
"  1. Write Cubeiform with to_solid() wrapper\n"
"  2. Render (F5 or preview_render) to populate voxel grid\n"
"  3. File > Export > Export as STL (GUI) or:\n"
"     marching_cubes <path.stl> (inspect command)\n"
"  4. Result: binary STL ready for 3D printing\n";

static const char HELP_INSPECT_CMD_BEZIER[] =
"INSPECT: COMMANDS / BEZIER -- Bezier Mesh and 2D Commands\n"
"\n"
"BEZIER PATCH MESH COMMANDS:\n"
"  bezier_mesh_new <r> <c>      Create flat RxC patch mesh\n"
"  bezier_mesh_sphere [rad] [res] Sphere (2x3 patches)\n"
"  bezier_mesh_torus [R] [r] [rows] [cols] [res]\n"
"  bezier_mesh_box [w] [h] [d] [res]  Box mesh\n"
"  bezier_mesh_cylinder [args]   Cylinder mesh\n"
"  bezier_mesh_set_cp <r> <c> <x> <y> <z>  Set control point\n"
"  bezier_mesh_state            JSON: dims, bbox, mode, resolution\n"
"  bezier_mesh_view <mode>      wireframe|voxel|both|none\n"
"  bezier_mesh_resolution <n>   Set voxel resolution\n"
"  bezier_mesh_clear            Free mesh, clear viewport\n"
"  bezier_mesh_cp_list          JSON array of all CPs\n"
"  bezier_mesh_loops            List all row/col loops\n"
"  bezier_mesh_select_loop <row|col> <i>  Select + highlight\n"
"  bezier_mesh_selected         Get selected loop info\n"
"  bezier_mesh_loop_cps         JSON of selected loop's CPs\n"
"  bezier_mesh_edit_loop        Open loop in 2D editor\n"
"  bezier_mesh_apply_loop       Write 2D edits back to 3D\n"
"  bezier_mesh_cancel_loop      Cancel loop editing\n"
"\n"
"BEZIER 2D COMMANDS:\n"
"  bezier_2d_capture [path]     Capture 2D canvas to PNG\n"
"  bezier_2d_projection         Get 2D projection info\n"
"  bezier_2d_move_point <args>  Move point in 2D editor\n"
"\n"
"BEZIER PRIMITIVES:\n"
"  bezier_sphere [args]         Create bezier sphere\n"
"  bezier_torus [args]          Create bezier torus\n"
"  bezier_triclaude [args]      Create Triclaude shape\n";

static const char HELP_INSPECT_CMD_EDA[] =
"INSPECT: COMMANDS / EDA -- Schematic, PCB, Library Commands\n"
"\n"
"EDA SCHEMATIC COMMANDS:\n"
"  sch_state                    Schematic state JSON\n"
"  sch_load <path>              Load schematic file\n"
"  sch_save <path>              Save schematic file\n"
"  sch_export_dcad [path]       Export as Cubeiform source\n"
"  sch_add_symbol <args>        Add symbol to schematic\n"
"  sch_add_wire <args>          Add wire\n"
"  sch_add_label <args>         Add label\n"
"  sch_select <args>            Select component\n"
"  sch_zoom <level>             Set schematic zoom\n"
"  sch_pan <x> <y>              Set schematic pan\n"
"  sch_render [path]            Render schematic to PNG\n"
"\n"
"EDA PCB COMMANDS:\n"
"  pcb_state                    PCB state JSON\n"
"  pcb_load <path>              Load PCB file\n"
"  pcb_save <path>              Save PCB file\n"
"  pcb_add_track <args>         Add track\n"
"  pcb_add_via <args>           Add via\n"
"  pcb_add_footprint <args>     Add footprint\n"
"  pcb_layer <n>                Set active layer\n"
"  pcb_layer_toggle <n>         Toggle layer visibility\n"
"  pcb_ratsnest                 Show ratsnest\n"
"  pcb_import_netlist           Import netlist from schematic\n"
"  pcb_export_dcad [path]       Export as Cubeiform source\n"
"  pcb_render [path]            Render PCB to PNG\n"
"\n"
"EDA LIBRARY COMMANDS:\n"
"  eda_lib_list                 List loaded libraries\n"
"  eda_lib_symbols <lib>        List symbols in library\n"
"  eda_sym_preview <sym> [path] Preview symbol as PNG\n"
"  eda_sym_info <sym>           Symbol pin/property info\n"
"  eda_fp_list                  List all footprints\n"
"  eda_fp_lib_list              List footprint libraries\n"
"  eda_fp_lib_footprints <lib>  List footprints in library\n"
"  eda_fp_preview <fp> [path]   Preview footprint as PNG\n";
...
"  Phase 2  COMPLETE   Bezier tool (canvas, editor, export)\n"
"  Phase 3  COMPLETE   OpenSCAD IDE (code editor, GL viewport)\n"
"  EDA E1   COMPLETE   S-expr parser + EDA data model\n"
"  EDA E2   COMPLETE   Cubeiform EDA + tab system + sch canvas\n"
"  EDA E3   COMPLETE   PCB layout canvas + routing + ratsnest\n"
"  EDA E4   NEXT       Assembly bridge (PCB↔3D constraints)\n"
"  EDA E5   PLANNED    Validation + manufacturing output\n"
"  Phase 4  PLANNED    3D assembly viewport\n"
"  Phase 5  PARTIAL    KiCad bridge (native EDA done, needs STEP)\n"
...
"  char      *dc_sexpr_write_pretty(node, err) KiCad-style indented\n"
...
"\n"
"CREATION (Phase 4A):\n"
"  dc_sexpr_new_atom/string/list()            create nodes\n"
"  dc_sexpr_clone(node)                       deep copy\n"
"\n"
"MUTATION:\n"
"  dc_sexpr_add_child(parent, child)          append child to list\n"
"  dc_sexpr_remove_child(parent, index)       remove + free child\n"
"  dc_sexpr_replace_child(parent, i, new)     swap child\n"
"  dc_sexpr_set_value(node, val)              change atom/string value\n"
"\n"
"DESIGN:\n"
...
"  dc_elibrary_load_footprint_dir(lib, dir)  scan .pretty dir\n"
"  dc_elibrary_find_symbol(lib, lib_id)      lookup by lib:name\n"
"  dc_elibrary_find_symbol_by_name(lib, name) name-only search\n"
"  dc_elibrary_find_footprint(lib, lib_id)   footprint lookup\n"
"  dc_elibrary_symbol_count/symbol_name       enumerate all symbols\n"
"  dc_elibrary_lib_count/lib_name             enumerate loaded libs\n"
"  dc_elibrary_lib_symbol_count/name          per-lib symbol iteration\n"
"  dc_elibrary_footprint_count/name/lib_name  footprint enumeration\n"
"  dc_elibrary_symbol_property(sym, key)      extract sexpr property\n"
"  dc_elibrary_symbol_pin_count(sym)          count pins across units\n"
"\n"
"DESIGN:\n"
...
"\n"
"SCHEMATIC CANVAS (src/eda_ui/sch_canvas.h/.c):\n"
"  Interactive canvas with mode-aware gesture dispatch.\n"
"  Button 1: select/place (mode-dependent), Button 2: drag-pan\n"
"  Hit testing: symbols (bbox), wires (point-to-segment),\n"
"    junctions (radius), labels (text bbox), power ports (radius)\n"
"  Selection: click to select, drag to move, highlight selected\n"
"  Wire drawing: W key -> click chain, auto-junction, dbl-click to end\n"
"  Overlay: crosshair cursor, wire preview (green dashed)\n"
"  Keyboard: Esc=Select, W=Wire, A=Symbol, L=Label, M=Move,\n"
"    R=Rotate, X=Mirror, Del=Delete\n"
"\n"
"SCHEMATIC EDITOR (src/eda_ui/sch_editor.h/.c):\n"
"  Toolbar: Select, Wire, Symbol, Label, Move buttons\n"
"  Modes: SELECT, WIRE, PLACE_SYMBOL, PLACE_LABEL, MOVE\n"
"  File I/O: load/save .kicad_sch\n"
"\n"
"PCB CANVAS (src/eda_ui/pcb_canvas.h/.c):\n"
"  Interactive multi-layer canvas with mode-aware dispatch.\n"
"  Hit testing: footprints (bbox), tracks (segment+width),\n"
"    vias (radius), pads (rect), zones (bbox)\n"
"  Route drawing: X key -> click chain, via insertion with V,\n"
"    auto-net from pad, dbl-click to end\n"
"  Selection: click to select, drag to move, R=rotate, F=flip,\n"
"    Del=delete, +/-=layer switch\n"
"  Overlay: crosshair, route preview (layer-colored, track-width)\n"
"  Keyboard: Esc=Select, X=Route, V=Via, F=Flip, M=Move,\n"
"    R=Rotate, Del=Delete, +/-=Layer\n"
"\n"
"PCB EDITOR (src/eda_ui/pcb_editor.h/.c):\n"
"  Toolbar: Select, Route, Via, Footprint buttons\n"
"  Modes: SELECT, ROUTE, PLACE_FOOTPRINT, PLACE_VIA, ZONE, MEASURE\n"
"\n"
"SYMBOL LIBRARY BROWSER (src/eda_ui/eda_library_browser.h/.c):\n"
"  Three-pane: Libraries | Symbols | Preview + Info\n"
"  dc_eda_library_browser_run(parent, lib, kind) -> lib_id string\n"
"  Cross-library search, per-lib browsing, Cairo symbol preview\n"
"\n"
"FOOTPRINT BROWSER (src/eda_ui/eda_footprint_browser.h/.c):\n"
"  Three-pane: Libraries | Footprints | Preview + Info\n"
"  dc_eda_footprint_browser_run(parent, lib) -> lib:fp string\n"
"  Layer-colored footprint preview (pads, silkscreen, courtyard)\n"
"\n"
"FOOTPRINT RENDERER (src/eda_ui/pcb_footprint_render.h/.c):\n"
"  dc_pcb_footprint_render_preview(cr, fp_def, x, y, w, h)\n"
"  Standalone Cairo rendering: pad/fp_line/fp_rect/fp_circle/fp_arc\n"
"\n"
"SYMBOL EDITOR (src/eda_ui/sym_editor.h/.c):\n"
"  dc_sym_editor_run(parent, sym_def, lib_path, lib) -> saved\n"
"  Properties panel + canvas, add rect/line/circle/pin, save-back\n"
"\n"
"FOOTPRINT EDITOR (src/eda_ui/fp_editor.h/.c):\n"
"  dc_fp_editor_run(parent, fp_def, fp_path) -> saved\n"
"  Pad list + properties, add pad, layer-colored preview, save-back\n"
"\n"
"OTHER FILES:\n"
"  src/eda_ui/sch_symbol_render.h/.c Symbol rendering to Cairo\n"
"    + dc_sch_symbol_render_preview() standalone preview (no canvas)\n"
"  src/eda_ui/pcb_layer_panel.h/.c   Layer visibility sidebar\n"
"  src/ui/eda_view.h/.c              EDA tab container (paned layout)\n"
"  src/ui/app_window.c               GtkStack tab system\n"
"\n"
"DATA MODEL MUTATIONS:\n"
"  Schematic: add/remove symbol, wire, label, junction, power_port\n"
"  PCB: add/remove footprint, track, via, zone\n"
"\n"
"INSPECT COMMANDS:\n"
"  sch_state/load/save/export_dcad/add_*/select/zoom/pan/render\n"
"  pcb_state/load/save/add_*/layer/layer_toggle/ratsnest/import/export\n"
"  pcb_render <path> [w] [h]      Render PCB canvas to PNG\n"
"  eda_lib_list                    JSON array of library names\n"
"  eda_lib_symbols <lib>           JSON array of symbols in lib\n"
"  eda_sym_preview <lib_id> [path] [w] [h]  Render symbol to PNG\n"
"  eda_sym_info <lib_id>           Symbol properties + pin count\n"
"  eda_fp_preview <lib_id> [path] [w] [h]   Render footprint to PNG\n"
"  eda_fp_list                     JSON array of loaded footprints\n"
"\n"
"RATSNEST ENGINE:\n"
...
static const char HELP_AGENT[] =
"AGENT -- How to Use DunCAD as an AI Agent\n"
"\n"
"DunCAD is controlled via inspect commands sent to /tmp/duncad.sock.\n"
"Use socat or duncad-inspect to send commands. All responses are JSON.\n"
"\n"
"TOPICS:\n"
"  duncad-docs agent render    How to render geometry and see it\n"
"  duncad-docs agent verify    Visual inspection protocol\n"
"  duncad-docs agent export    How to export STL for 3D printing\n"
"  duncad-docs agent pitfalls  Common mistakes and how to avoid them\n"
"\n"
"QUICK START:\n"
"  1. echo 'set_code sphere(10);' | socat - UNIX:/tmp/duncad.sock\n"
"  2. echo 'preview_render' | socat - UNIX:/tmp/duncad.sock\n"
"  3. sleep 3  (wait for render)\n"
"  4. echo 'render_status' | socat - UNIX:/tmp/duncad.sock\n"
"  5. echo 'gl_capture /tmp/view.png' | socat - UNIX:/tmp/duncad.sock\n"
"  6. View /tmp/view.png to verify\n";

static const char HELP_AGENT_RENDER[] =
"AGENT: RENDER -- How to Render Geometry\n"
"\n"
"DunCAD renders geometry natively via SDF (signed distance fields).\n"
"No triangles. No external OpenSCAD process. All rendering is native.\n"
"\n"
"STEP 1: SET CODE\n"
"  echo 'set_code <source>' | socat - UNIX:/tmp/duncad.sock\n"
"\n"
"  Source can be Cubeiform or OpenSCAD syntax. Both work natively.\n"
"  Examples:\n"
"    set_code sphere(10);                    -- single sphere\n"
"    set_code cube(10) >> move(x=5);         -- Cubeiform pipe syntax\n"
"    set_code cube([10,20,5]);               -- OpenSCAD array syntax\n"
"    set_code difference() { cube(10); sphere(12); }  -- CSG\n"
"\n"
"STEP 2: RENDER\n"
"  echo 'preview_render' | socat - UNIX:/tmp/duncad.sock\n"
"\n"
"  This triggers an F5 render. The geometry is parsed, SDF fields\n"
"  are computed, and the result appears in the GL viewport.\n"
"  Rendering is asynchronous — poll with render_status.\n"
"\n"
"STEP 3: WAIT FOR COMPLETION\n"
"  echo 'render_status' | socat - UNIX:/tmp/duncad.sock\n"
"  -> {\"rendering\":false,\"status\":\"Rendered: 113060 active...\"}\n"
"\n"
"  Wait until rendering is false. Typical render: 1-5 seconds.\n"
"\n"
"STEP 4: VISUALLY INSPECT\n"
"  echo 'gl_capture /tmp/view.png' | socat - UNIX:/tmp/duncad.sock\n"
"\n"
"  This captures the 3D GL viewport to a PNG file. READ THIS FILE\n"
"  to see what the geometry looks like. This is the ONLY way to\n"
"  verify your work. Do not skip this step.\n"
"\n"
"CAMERA CONTROL:\n"
"  gl_camera <dist> <yaw> <pitch>   Set camera angle\n"
"  gl_reset                         Reset to default view\n"
"  gl_ortho                         Toggle orthographic\n"
"\n"
"RENDER MODES:\n"
"  The viewport renders SDF analytically (infinite smooth surfaces)\n"
"  by default. For voxelized output (needed for STL export), wrap\n"
"  your code in to_solid():\n"
"    set_code to_solid(sphere(10));\n"
"\n"
"MESH DIAGNOSTIC (explicit-only):\n"
"  preview_render is the holy SDF/voxel path — never produces\n"
"  triangles in the viewport. If you need to compare against\n"
"  the OpenSCAD-mesh interpretation (e.g., debugging why an SDF\n"
"  primitive looks different from its OpenSCAD output), use:\n"
"    debug_render_mesh [path.stl]\n"
"  This loads triangles into the MESH canvas with a [DEBUG MESH]\n"
"  banner so the output is unmistakable. Never fires automatically.\n"
"\n"
"SEE ALSO:\n"
"  duncad-docs gl modes        Three view modes proof-of-purity\n"
"  duncad-docs agent verify    Visual inspection checklist\n"
"  duncad-docs agent pitfalls  Common mistakes\n";

static const char HELP_AGENT_VERIFY[] =
"AGENT: VERIFY -- Visual Inspection Protocol\n"
"\n"
"THE FIFTH COMMANDMENT: Verify with your own eyes before declaring\n"
"victory. Tests are necessary but not sufficient.\n"
"\n"
"MANDATORY WORKFLOW:\n"
"  1. gl_capture /tmp/view.png   -- capture viewport\n"
"  2. READ the PNG file          -- actually look at it\n"
"  3. ENUMERATE what you see     -- never say 'looks good'\n"
"  4. COMPARE to what you expect -- flag discrepancies\n"
"\n"
"THE SIX VIEWS (for thorough inspection):\n"
"  Front:   gl_camera <dist> 0 0     then gl_capture\n"
"  Right:   gl_camera <dist> 90 0    then gl_capture\n"
"  Top:     gl_camera <dist> 0 90    then gl_capture\n"
"  Back:    gl_camera <dist> 180 0   then gl_capture\n"
"  Left:    gl_camera <dist> 270 0   then gl_capture\n"
"  Bottom:  gl_camera <dist> 0 -90   then gl_capture\n"
"\n"
"  Use gl_ortho for orthographic views (more accurate).\n"
"\n"
"CHECKLIST (for each view):\n"
"  [ ] Can I see interior through exterior?  -> HOLE\n"
"  [ ] Is any element disconnected?          -> FLOATING\n"
"  [ ] Do mating surfaces touch?             -> Z-GAP\n"
"  [ ] Are all expected elements visible?    -> MISSING\n"
"\n"
"NEVER say 'looks correct' or 'as expected'.\n"
"ALWAYS enumerate: 'I see X, Y, Z. I do NOT see W.'\n";

static const char HELP_AGENT_EXPORT[] =
"AGENT: EXPORT -- How to Export STL for 3D Printing\n"
"\n"
"STL export requires a voxel grid. The pipeline:\n"
"\n"
"  1. Write Cubeiform with to_solid() wrapper:\n"
"     set_code to_solid(sphere(10));\n"
"\n"
"  2. Render to populate voxel grid:\n"
"     preview_render\n"
"     (wait for render_status to show rendering:false)\n"
"\n"
"  3. Export via marching cubes:\n"
"     marching_cubes /path/to/output.stl\n"
"\n"
"  4. Verify the STL (load it back and look):\n"
"     gl_load /path/to/output.stl\n"
"     gl_capture /tmp/stl_check.png\n"
"\n"
"  Or use the GUI: File > Export > Export as STL\n"
"\n"
"RESOLUTION:\n"
"  Higher $vd = more voxels = smoother STL = slower.\n"
"  $vd = 3 is preview quality. $vd = 6-8 for print.\n"
"  set_code $vd = 6;\\nto_solid(sphere(10));\n"
"\n"
"VERIFY BEFORE SENDING TO SLICER:\n"
"  After exporting, load the STL back with gl_load and\n"
"  visually inspect. This shows you exactly what the\n"
"  slicer will receive.\n";

static const char HELP_AGENT_PITFALLS[] =
"AGENT: PITFALLS -- Common Mistakes\n"
"\n"
"1. USING 'render' INSTEAD OF 'gl_capture'\n"
"   render [path]    captures the 2D BEZIER CANVAS (not 3D)\n"
"   gl_capture       captures the 3D GL VIEWPORT (what you want)\n"
"   These are different. You almost always want gl_capture.\n"
"\n"
"2. USING 'render_scad' INSTEAD OF 'preview_render'\n"
"   render_scad      uses the OLD OpenSCAD subprocess (legacy)\n"
"   preview_render   uses the NATIVE SDF engine (correct)\n"
"   Always use preview_render.\n"
"\n"
"3. NOT WAITING FOR RENDER TO COMPLETE\n"
"   preview_render is asynchronous. After calling it, poll\n"
"   render_status until rendering:false before capturing.\n"
"\n"
"4. NOT VISUALLY INSPECTING\n"
"   gl_capture -> read the PNG. Every time. No exceptions.\n"
"   Never declare geometry correct without looking at it.\n"
"\n"
"5. GENERATING .scad FILES INSTEAD OF USING THE INTERFACE\n"
"   Do NOT write .scad files and call openscad externally.\n"
"   Use set_code + preview_render. Everything renders natively.\n"
"\n"
"6. FORGETTING to_solid() FOR STL EXPORT\n"
"   Analytical SDF rendering does not produce a voxel grid.\n"
"   To export STL, wrap code in to_solid():\n"
"     to_solid(cube(10));    -- produces voxels -> exportable\n"
"     cube(10);              -- SDF only -> not exportable\n"
"\n"
"7. USING cubeiform_exec INSTEAD OF set_code + preview_render\n"
"   cubeiform_exec runs code through inspect, populating the\n"
"   inspect module's voxel grid. But it does NOT update the\n"
"   GL viewport via the preview pipeline. Use set_code to put\n"
"   code in the editor, then preview_render to render it.\n";


static const char HELP_GL[] =
"GL -- OpenGL ES 3.2 Render Layer\n"
"\n"
"src/gl/ contains the rendering primitives that draw the 3D\n"
"viewport. The viewport (gl_viewport.c) is the conductor; the\n"
"specialized raycasters (gl_sdf_analytical.c, gl_voxel.c,\n"
"gl_bezier_ray.c) are the instruments that draw the three\n"
"holy view modes.\n"
"\n"
"TOPICS:\n"
"  duncad-docs gl modes      The three view modes, shader-by-shader\n"
"\n"
"FILES:\n"
"  src/gl/gl_viewport.c       Window, camera, render dispatch\n"
"  src/gl/gl_sdf_analytical.c smooth-mode raycaster (analytical SDF)\n"
"  src/gl/gl_voxel.c          surface + blocky raycaster (3D texture)\n"
"  src/gl/gl_bezier_ray.c     smooth bezier-direct raycaster (CP uniforms)\n"
"  src/gl/gl_bezier_wire.c    bezier wireframe overlay\n"
"  src/gl/stl_loader.c        STL importer (reference STLs only)\n";

static const char HELP_GL_MODES[] =
"GL: MODES -- Three Views, One Truth (HP6 Phase 5 audit)\n"
"\n"
"DunCAD has THREE holy view modes. Each samples a different\n"
"projection of the same SDF/voxel truth. None samples a\n"
"mesh-rasterized FBO. Each was read shader-by-shader and\n"
"verified pure during HP6 Phase 5.\n"
"\n"
"MODE 1: smooth (analytical SDF raycast)\n"
"  Shader:   src/gl/gl_sdf_analytical.c FRAG_SRC\n"
"  Inputs:   uPrimType[64], uPrimCSG[64], uPrimPos[64],\n"
"            uPrimSize[64], uPrimExtra[64], uPrimColor[64]\n"
"            (uniform arrays — primitive parameters)\n"
"  Method:   evalSDF() raymarches via length()/abs()/min()/max()\n"
"            on primitive parameters. Normals via central\n"
"            differences on the analytical SDF.\n"
"  Texture samplers: NONE.\n"
"  FBO read: NONE.\n"
"  Verdict:  PURE. The visible chain ends at the SDF\n"
"            function tree itself.\n"
"\n"
"MODE 1b: smooth (bezier-direct raycast)\n"
"  Shader:   src/gl/gl_bezier_ray.c FRAG_SRC1+2+3\n"
"  Inputs:   uCP[216] (uniform array — control points)\n"
"  Method:   evalP/evalDu/evalDv evaluate biquadratic bezier\n"
"            polynomials. hitPatch uses Newton's method.\n"
"            Normals from analytical derivatives (cross of\n"
"            Su, Sv) refined by screen-space dFdx/dFdy.\n"
"  Texture samplers: NONE.\n"
"  Verdict:  PURE. Math on control points; no mesh.\n"
"\n"
"MODE 2: surface (voxel SDF, smooth interpolation)\n"
"  Shader:   src/gl/gl_voxel.c FRAG_SRC + FRAG_SRC2\n"
"            (smooth branch: uBlocky == 0)\n"
"  Inputs:   sampler3D uSDF, sampler3D uColor\n"
"  Method:   sphere-trace; sdf() does smoothstep-weighted\n"
"            texelFetch on uSDF for C1 continuity. Normals\n"
"            via texture(uSDF,...) central differences\n"
"            (trilinear). Color: smoothstep-weighted\n"
"            texture(uColor,...) at hit point.\n"
"  FBO read: NONE.\n"
"  Verdict:  PURE. Samples voxel 3D textures only.\n"
"\n"
"MODE 3: blocky (voxel grid, nearest sample)\n"
"  Shader:   src/gl/gl_voxel.c FRAG_SRC + FRAG_SRC2\n"
"            (blocky branch: uBlocky != 0)\n"
"  Inputs:   sampler3D uSDF, sampler3D uColor (same textures)\n"
"  Method:   DDA cell traversal. texelFetch(uSDF, cell, 0)\n"
"            checks <= 0.0 per cell. Face normals derived\n"
"            from DDA step axis. Color: texelFetch(uColor,\n"
"            cell, 0).rgb at the hit cell.\n"
"  FBO read: NONE.\n"
"  Verdict:  PURE. Samples voxel 3D textures only.\n"
"\n"
"RENDER FLOW (gl_viewport.c on_render):\n"
"  1. Clear buffers, build view/proj matrices\n"
"  2. Upload pending mesh / objects / voxel grid\n"
"  3. Draw grid (line_prog)\n"
"  4. Draw mesh (mesh_prog) — ONLY if obj_count > 0 or\n"
"     legacy mesh present. Dormant during pure-Cubeiform\n"
"     renders.\n"
"  5. Wireframe / face / edge / bezier-wire overlays\n"
"  6. SDF raycast dispatch (one of three):\n"
"       - dc_gl_sdf_draw            (analytical, MODE 1)\n"
"       - dc_gl_bezier_ray_draw     (bezier-direct, MODE 1b)\n"
"       - dc_gl_voxel_buf_draw      (MODE 2 or MODE 3)\n"
"  7. Draw axes (line_prog, two-pass)\n"
"\n"
"THE MESH SHADER (mesh_prog):\n"
"  Lives at gl_viewport.c MESH_VERT_SRC / MESH_FRAG_SRC.\n"
"  Standard Phong on triangle attributes. Fires only when\n"
"  vp->obj_count > 0 or vp->mesh != NULL — i.e., when STLs\n"
"  are explicitly loaded (gl_load <path>) or the legacy\n"
"  mesh canvas is active. Never fired by the three holy\n"
"  view modes. Imported reference STLs render alongside\n"
"  the SDF view but are not its source.\n"
"\n"
"THE PICK FBO:\n"
"  gl_viewport.c uses an FBO ONLY for color-ID object\n"
"  picking on click. Read on mouse events. NEVER read\n"
"  back into a render shader. Not part of any view mode.\n"
"\n"
"PROOF: grep -E 'sampler|texture|FBO' src/gl/*.c\n"
"  shows uSDF / uColor are the only samplers used in the\n"
"  three view-mode shaders, and they are 3D textures\n"
"  uploaded from the voxel grid (not from any FBO).\n"
"\n"
"SEE ALSO:\n"
"  talmud reference doctrine holy-path-vi  Phase 5 charter\n"
"  duncad-docs voxel                       voxel grid lifecycle\n";
...
static const struct help_node TREE[] = {
    ...
    { "inspect.commands.core",  HELP_INSPECT_CMD_CORE },
    { "inspect.commands.gl",    HELP_INSPECT_CMD_GL },
    { "inspect.commands.voxel", HELP_INSPECT_CMD_VOXEL },
    { "inspect.commands.bezier",HELP_INSPECT_CMD_BEZIER },
    { "inspect.commands.eda",   HELP_INSPECT_CMD_EDA },
    ...
    /* agent */
    { "agent",                   HELP_AGENT },
    { "agent.render",            HELP_AGENT_RENDER },
    { "agent.verify",            HELP_AGENT_VERIFY },
    { "agent.export",            HELP_AGENT_EXPORT },
    { "agent.pitfalls",          HELP_AGENT_PITFALLS },
