...
"   }\n"
"\n"
"7. HEMISPHERE — difference(intersection) SILENTLY FAILS:\n"
"   This pattern produces NO geometry in Trinity Site:\n"
"     difference() {\n"
"         intersection() { sphere(R); [half-space cube]; }\n"
"         intersection() { sphere(R-t); [half-space cube]; }\n"
"     }\n"
"   Use rotate_extrude() of a 2D arc polygon instead:\n"
"     rotate_extrude($fn=N)\n"
"         polygon(concat(\n"
"             [for (i=[0:ARC]) [R*cos(i*90/ARC),   R*sin(i*90/ARC)]],\n"
"             [for (i=[ARC:-1:0]) [(R-t)*cos(i*90/ARC),(R-t)*sin(i*90/ARC)]]\n"
"         ));\n"
"   This creates a hollow hemisphere shell correctly.\n"
"\n"
"8. rotate_extrude() POLYGON ARC PATTERN:\n"
"   Dome shell (outer R, inner R-t, upper hemisphere only):\n"
"     ARC = 48;  // steps, higher = smoother\n"
"     rotate_extrude($fn=96)\n"
"         polygon(concat(\n"
"             [for(i=[0:ARC]) [R*cos(i*90/ARC), R*sin(i*90/ARC)]],\n"
"             [for(i=[ARC:-1:0]) [(R-t)*cos(i*90/ARC),(R-t)*sin(i*90/ARC)]]\n"
"         ));\n"
"   Works because rotate_extrude sweeps 2D profile around Z axis.\n"
"   XY = radial, Y = height in rotate_extrude context.\n"
"\n"
"9. rotate([0,90,0]) linear_extrude() SILENTLY FAILS:\n"
"   Wrapping linear_extrude in a 3D rotate produces no geometry:\n"
"     rotate([0,90,0]) linear_extrude(h=10) square([5,8]);  // BROKEN\n"
"   Use hull() of perpendicular thin cubes instead:\n"
"     // Diamond prism oriented in X (radially through cylinder wall):\n"
"     hull() {\n"
"         cube([depth, 0.1, d_h], center=true);  // spans Z\n"
"         cube([depth, d_w,  0.1], center=true); // spans Y\n"
"     }\n"
"   hull() of two perpendicular slabs = rhombus cross-section prism.\n"
"   Works for any orientation — no linear_extrude needed.\n"
"\n"
"10. CUBEIFORM for-loop CSG LIMITATIONS (native voxel renderer):\n"
"    for loops in Cubeiform work as top-level statements but FAIL\n"
"    when used as CSG operands:\n"
"      base - for i in [0:n] { cutter; }   // FAILS: unknown voxel var\n"
"      difference() { base; for i in ... } // FAILS: No solid geometry\n"
"    Workaround for parametric hole patterns: write in OpenSCAD style.\n"
"    OpenSCAD for() + difference() render correctly via OpenSCAD fallback.\n"
"    The native voxel renderer also cannot handle large complex CSG\n"
"    (big cylinders with many hull()-cut holes produce 0 active voxels).\n"
"    Use the OpenSCAD fallback path for complex parametric designs.\n";
