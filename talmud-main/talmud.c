...
"\n"
"TARGETS:  talmud, yotzer, darshan, sofer\n"
"\n"
"SELF-REEXEC: If yotzer.c is modified, yotzer detects staleness on\n"
"next run, recompiles itself, and re-execs. No manual re-bootstrap\n"
"needed -- only for a truly fresh clone.\n";

static const char HELP_TOOLS_YOTZER_ALL[] =
"Build Everything\n"
"\n"
"  yotzer all              Build all 4 targets, install to ~/.local/bin\n";

static const char HELP_TOOLS_YOTZER_SINGLE[] =
"Build a Single Target\n"
"\n"
"  yotzer <target>         Build a single target by name\n"
"                          Valid targets: talmud, yotzer, darshan, sofer\n";

static const char HELP_TOOLS_YOTZER_CLEAN[] =
"Remove Build Artifacts\n"
"\n"
"  yotzer clean            Remove all compiled binaries\n";

static const char HELP_TOOLS_YOTZER_INSTALL[] =
"Install to PATH\n"
"\n"
"  yotzer install          Copy compiled binaries to ~/.local/bin\n"
"                          Called automatically by yotzer all\n";
...
"\n"
"OVERVIEW:\n"
"  Fixed the broken right-click Modify Shape menu, added transform\n"
"  panel integration, Enter-to-render, 3D print parameters, fixed\n"
"  the stale line range trap for chained modifications, and wrote\n"
"  the SCAD operation ordering doctrine into the sacred texts.\n"
"\n"
"BUG FIX: MODIFY SHAPE MENU NOT FIRING (shape_menu.c):\n"
"  Root cause: GtkPopoverMenu with GMenuModel cannot resolve\n"
"  action groups from parent widget hierarchy on GtkGLArea in\n"
"  GTK4/Wayland. Menu items rendered but clicking did nothing.\n"
"  Fix: Replaced GtkPopoverMenu entirely with plain GtkPopover\n"
"  containing GtkButton widgets with direct \"clicked\" signal\n"
"  callbacks, bypassing GTK4's broken action resolution.\n"
"  Three attempts: (1) capture selection at right-click — no fix;\n"
"  (2) install action group on popover — no fix; (3) direct\n"
"  buttons — FIXED.\n"
"\n"
"FEATURE: TRANSFORM PANEL INTEGRATION:\n"
"  When adding translate or rotate via Modify Shape, the on-screen\n"
"  transform panel (X/Y/Z entry fields) now appears automatically.\n"
"  ShapeMenuCtx gained DC_TransformPanel* and DC_ScadPreview*\n"
"  fields. After wrap_selected(), panel is shown with new statement.\n"
"\n"
"FEATURE: ENTER-TO-RENDER:\n"
"  Added DC_TransformEnterCb typedef and setter to transform_panel.\n"
"  Connected GtkEntry \"activate\" signal on all 6 entry widgets.\n"
"  Pressing Enter in any transform field triggers dc_scad_preview_render.\n"
"  Wired in app_window.c via on_transform_enter callback.\n"
"\n"
"FEATURE: 3D PRINT PARAMETERS:\n"
"  Added $fn=100; $fa=1; $fs=0.4; as preamble variables in the\n"
"  default SCAD template.\n"
"\n"
"BUG FIX: STALE LINE RANGE TRAP (shape_menu.c) — VERIFIED:\n"
"  Two sub-bugs found and fixed:\n"
"  1. wrap_selected() didn't update sel_line_end after modifying code.\n"
"     Fix: count newlines in new buffer, update ctx->sel_line_end.\n"
"  2. Viewport fallback path in get_selected_line_range() queried\n"
"     the viewport but didn't capture into ctx. So even though\n"
"     wrap_selected updated sel_line_end, the NEXT call went back\n"
"     to the viewport's stale lines. Fix: capture sel_obj,\n"
"     sel_line_start, sel_line_end into ctx on viewport fallback.\n"
"  Verified via inspect: cube -> difference -> translate -> rotate\n"
"  chains correctly, each wrapping the full preceding statement.\n"
"  Log confirms: 3..3 -> 3..6 -> 3..8 -> 3..10.\n"
"\n"
"BUG FIX: INDENTATION BUFFER OVERFLOW (shape_menu.c):\n"
"  wrap_selected() allocated sel_len*2+1 for the indented buffer.\n"
"  Each newline adds 4 spaces, so multi-line text overflows.\n"
"  Crashed with ASan heap-buffer-overflow at line 164.\n"
"  Fix: allocate sel_len*5+8 to handle worst case.\n"
"\n"
"TALMUD: SCAD OPERATION ORDERING DOCTRINE:\n"
"  New node: reference.architecture.scad.ordering\n"
"  Title: \"The Law of Nesting\"\n"
"  Documents: three classes of operation (transforms, booleans,\n"
"  geometry), the Cardinal Rule (transforms wrap everything),\n"
"  correct vs wrong nesting examples, nesting order, the stale\n"
"  line range trap, and chaining rules.\n"
"\n"
"FILES MODIFIED:\n"
"  src/ui/shape_menu.c        Rewritten: direct buttons, line range fix,\n"
"                             viewport capture, buffer overflow fix\n"
"  src/ui/shape_menu.h        Updated API: +transform, +preview params\n"
"  src/ui/transform_panel.c   Enter callback support (activate signal)\n"
"  src/ui/transform_panel.h   DC_TransformEnterCb typedef + setter\n"
"  src/ui/app_window.c        Wired transform panel + enter-to-render\n"
"  src/core/log.c             Added fflush(stderr) after each log write\n"
"  src/inspect/inspect.c      Added shape_action inspect command\n"
"  talmud.c                   SCAD ordering doctrine node\n"
"\n"
"LESSONS:\n"
"  - GtkPopoverMenu + GMenuModel action resolution is BROKEN on\n"
"    GtkGLArea/Wayland. Use direct GtkButton callbacks instead.\n"
"  - After programmatically modifying code, ALL cached line ranges\n"
"    must be updated or subsequent operations break silently.\n"
"  - When using a fallback query path, CAPTURE results into the\n"
"    persistent context or updates to derived fields will be lost.\n"
"  - Indentation buffers need worst-case sizing: each newline in\n"
"    the source adds 4 bytes of spaces to the output.\n"
"  - Transforms must wrap the ENTIRE statement including booleans.\n"
"  - stderr is fully buffered when redirected — need fflush().\n"
"  - calloc zeroes sel_obj to 0, which falsely indicates selection.\n"
"    Initialize to -1 explicitly.\n"
"\n"
"FOR THE NEXT AGENT:\n"
"  - All fixes verified via automated inspect test\n"
"  - SCAD ordering rules: talmud reference architecture scad ordering\n"
"  - Transform panel appears on translate/rotate, Enter renders\n";
static const struct help_node TREE[] = {
    ...
    { "tools.yotzer.all", HELP_TOOLS_YOTZER_ALL },
    { "tools.yotzer.single", HELP_TOOLS_YOTZER_SINGLE },
    { "tools.yotzer.clean", HELP_TOOLS_YOTZER_CLEAN },
    { "tools.yotzer.install", HELP_TOOLS_YOTZER_INSTALL },
